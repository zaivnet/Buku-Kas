<x-app-layout title="Tambah {{ $type === 'income' ? 'Pemasukan' : 'Pengeluaran' }}">
    <div class="max-w-2xl mx-auto" x-data="{ submitting: false }">
        {{-- Header --}}
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl font-bold text-neutral-text">
                    Tambah {{ $type === 'income' ? 'Pemasukan (Income)' : 'Pengeluaran (Expense)' }}
                </h1>
                <p class="text-sm text-neutral-muted mt-1">Catat transaksi keuangan kas baru ke dalam sistem.</p>
            </div>
        ...
        {{-- Card Form --}}
        <div class="card p-6">
            <form action="{{ route('transactions.store') }}" method="POST" enctype="multipart/form-data" x-on:submit="submitting = true">
